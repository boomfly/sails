...
module.exports = function (sails) {
    ...
    var Session = {
        ...
        fromSocket: function (socket, cb) {
            ...
            if (!socket.request.sessionID && !socket.headers.cookie) {
                ...
                socket.headers.cookie = generatedCookie;
                ...
                socket.cookie = cookie.parse(generatedCookie);
                // Parse and decrypt cookie and save it in the socket.handshake
                socket.request.sessionID = cookieParser.signedCookie(socket.cookie[sails.config.session.key], sails.config.session.secret);

                // Generate and persist a new session in the store
                Session.generate(socket.request, function (err, sessionData) {
                    if (err) return cb(err);
                    sails.log.silly('socket.request.sessionID is now :: ', socket.request.sessionID);
            ...
            try {
                ...
                // Parse and decrypt cookie and save it in the socket.handshake
                socket.request.sessionID = parseSignedCookie(socket.cookie[sails.config.session.key], sails.config.session.secret);
            } catch (e) {
