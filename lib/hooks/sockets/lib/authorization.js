module.exports = function(sails) {
	...
	return function socketAttemptingToConnect (sokect, accept) {

		// If a cookie override was provided in the query string, use it.
		// (e.g. ?cookie=sails.sid=a4g8dsgajsdgadsgasd)
		if (socket.query.cookie) {
			socket.headers.cookie = socket.query.cookie;
		}

		var sdk = getSDKMetadata(socket.headers.cookie);
		...
		sails.session.get(socket.request.sessionID, function(err, session) {
			...
			// So generate a new, blank session.
				...
				socket.request.session = new ConnectSession(handshake, {
					cookie: {
			...
			// Parsed cookie matches a known session- onward!
				...
				socket.request.session = new ConnectSession(socket.request, session);
