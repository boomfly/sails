...
module.exports = function (sails) {
  ...
  return function interpretSocketReq (socketReq, socketIOCallback, socket, messageName, cb) {
    ...
    var sdk = getSDKMetadata(socket.request.query);
    ...
    var forwarded = socket.headers['x-forwarded-for'];
    ...
    var req = {
      ...
      ip      : forwarded[0] || socket.address.ip,

      port  : forwarded[1] || socket.address.port,
    ...
    res.broadcast = function broadcastMessage (room, data) {
      req.socket.room(room).write(data);
      return res;
    };
