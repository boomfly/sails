...
module.exports = function (sails) {
    ...
    function addLowLevelSocketMethods() {
        ...
        sails.sockets.broadcast = function (roomName, eventName, data, socketToOmit) {
            ...
            if (socketToOmit) {
                sails.io.room(roomName).except(socketToOmit.id).send(eventName, data )
                // socketToOmit.broadcast.to(roomName).emit(eventName, data);
            }
            // Otherwise broadcast to everyone
            else {
                sails.io.room(roomName).write(eventName, data);
        ...
        sails.sockets.blast = function (eventName, data, socketToOmit) {
            ...
            if (socketToOmit) {
                ...
                socketToOmit.send(eventName, data);
            }

            // Otherwise broadcast to everyone
            else {
                sails.io.send(eventName, data);
            }
        };
        ...
        sails.sockets.emit = function (socketIDs, eventName, data) {
            ...
            _.each(socketIDs, function (socketID) {
                sails.io.spark(socketID).emit(eventName, data);
            });
        };
        ...
        sails.sockets.subscribers = function (roomName, returnSockets) {
            if (returnSockets) {
                var ids = sails.io.room(roomName).clients();
                var clients = [];
                for(var i = 0; i < ids.length; i++) {
                    clients.push(sails.io.spark(ids[i]));
                }
                return clients;
            } else {
                return _.pluck(sails.io.room(roomName).clients(), 'id');
            }
        };
        ...
        sails.sockets.socketRooms = function (socket) {
            return _.map(_.keys(socket.rooms()), function (roomName) {
        ...
        sails.sockets.rooms = function () {
            var rooms = sails.util.clone(sails.io.rooms());
            delete rooms[""];
            return sails.util.map(sails.util.keys(rooms), function (room) {
                ...
                // Otherwise publish to each instance room
                    ...
                    _.each(ids, function eachInstance(id) {
                        ...
                        if (sails.config.sockets['backwardsCompatibilityFor0.9SocketClients']) {
                            ...
                            if (legacyMsg.verb === 'created') {
                                legacyMsg.verb = 'create';
                            }
                            if (legacyMsg.verb === 'updated') {
                                legacyMsg.verb = 'update';
                            }
                            if (legacyMsg.verb === 'destroyed') {
                                legacyMsg.verb = 'destroy';
                            }
